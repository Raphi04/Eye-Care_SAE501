...
export default function CommentsSection({ subject }: CommentSectionProps) {
	...
	const [writedCommentLoadingState, setWritedCommentLoadingState] =
		useState<boolean>();
		...
		const getAllComments = async () => {
			try {
				...
				const response = await fetch(
					`http://localhost:8000/post/${subject}`,
					requestOptions
				);
							...
							<div className="writeComment">
								...
								<div className="sendCommentContainer">
									<div className="sendingInfo">
										{writedCommentLoadingState && (
											<p>Envoie du commentaire en cours...</p>
										)}
										...
										{writedCommentError && (
											<p className="error">
												Erreur lors de l'envoie du commentaire !
											</p>
										)}
									</div>
