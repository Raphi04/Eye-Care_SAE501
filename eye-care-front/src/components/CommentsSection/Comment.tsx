...
export default function Comment({
	...
	function getInitials(variable: any) {
		if (variable) {
			const usernameSplited = variable.username.split(" ");
	...
	function getDifferenceTime() {
		...
		const differenceSecondes = Math.round(differenceEnMs / 1000);
		const differenceMinutes = Math.round(differenceSecondes / 60);
		const differenceHeures = Math.round(differenceMinutes / 60);
		const differenceJours = Math.round(differenceHeures / 24);
		const differenceMois = Math.round(differenceJours / 30);
		const differenceAns = Math.round(differenceMois / 12);
	...
	function handleChangeReplyText(e: any) {
		setReplyError(false);
		if (isReply) {
			const mention = `@${commentData.username} `;
